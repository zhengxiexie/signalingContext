...
//#define		MAX_BCD_MSISDN_NUMBERS	15
#define		MAX_BCD_IMSI_LEN	8
#define		IMSI_LEN	15
//#define		MAX_IMSI_NUMBERS	8
...
typedef struct {
	...
	char hlr_table_name[50];
	char hlr_select_cond[100];
	char vlr_table_name[50];
	char vlr_select_cond[100];
	char roam_table_name[50];
	char roam_select_cond[100];	
	char host_code[21];
	char entity_type_code[21];
	char entity_sub_code[21];	
	int signal_check_interval;
	char pros_file_path[201];	
	char err_file_path[201];
  
  int signal_roam_judge_change;
	int signal_roam_judge_timing;		
} Config;
