...
int get_sleep_interval()
{
	...
	char sleep_interval[10];

	fp=fopen("../etc/ftp.conf","r");
	if(fp==NULL)
	{
		printf("文件[ftp.conf]打开错误！\n");
		exit(-1);
	}
	...
	{
		if(!strncmp(str, "sleep_interval", 14))
		{
			flag = 1;
	...
	if(flag == 1)
	{
		memset(sleep_interval,'\0',sizeof(sleep_interval));
		len=GetStr(str, sleep_interval, 1, '"');
		
		return atoi(sleep_interval);
	}
	else
	{
		printf("文件[ftp.conf]中未配置sleep_interval周期！\n");
...
int get_input_file_path(char *input_file_path)
{
	...
	while(fgets(str,200,fp)!=NULL)
	{
		if(!strncmp(str, "input_file_path", 15))
		{
			flag = 1;
	...
	if(flag == 1)
	{
		len=GetStr(str, input_file_path, 1, '"');
		
		return 0;
	}
	else
	{
		printf("文件[ftp.conf]中未配置input_file_path路径！\n");
...
int get_input_file_backup_path(char *input_file_backup_path)
{
	...
	while(fgets(str,200,fp)!=NULL)
	{
		if(!strncmp(str, "input_file_backup_path", 15))
		{
			flag = 1;
	...
	if(flag == 1)
	{
		len=GetStr(str, input_file_backup_path, 1, '"');
		
		return 0;
	}
	else
	{
		printf("文件[ftp.conf]中未配置input_file_backup_path路径！\n");
...
void *produce_fr_ftp(void *arg)
{
	...
	int len[50];
	char *word[50];
	char str_event_time[20],errinfo[200];
  ...
  SignalContent *end ;
	...
	strcpy(str_exit_file_name, "./saRoamMonitor.exitflag");
  ...
  //进入循环读取文件
	while(1){
		if((dp = opendir(input_file_path)) == NULL){
			   ...
			   printf("%s\n",errinfo);		   
			   write_error_file(chk.err_file_path,"saRoamMonitor","31",3,errinfo);
			   sleep(60);
			   continue;
		}else {
           	...
           	if( chk.signal_sort_flag == 1 ) {
                ...
                if(deal_time < 0){
                		 memset(errinfo,'\0',sizeof(errinfo));
			               ...
			               printf("%s\n",errinfo);			   
			               write_error_file(chk.err_file_path,"saRoamMonitor","32",2,errinfo);	
                }                 				    
				      	
				      	//找到当前文件排入的时段,根据文件名,
				      	//因为每分钟的数据文件不一定只有一个，所以需要先判断在索引是否存在
				      	//printf("str_deal_time=[%s]  deal_time=[%d],init_flag=[%d]\n",str_deal_time,deal_time,init_flag);
				      	if(init_flag == 1) {
				      		  time_t tmp_time;
				      		  tmp_time = deal_time;				      	    
            ...
            //判断信令是否需要排序 1-为需要，0-为不需要
                ...
                while( num > 0 ){
				      ...
				      pthread_mutex_lock(&pthread_logfile_mutex);			
					    ret = write_log_file(chk.log_file_path,"saRoamMonitor","71",str_file_name,str_end_time,msgvalue); 
		          pthread_mutex_unlock(&pthread_logfile_mutex);
                	...
                	memset(errinfo,'\0',sizeof(errinfo));
			            ...
			            printf("%s\n",errinfo);			   
			            write_error_file(chk.err_file_path,"saRoamMonitor","35",2,errinfo);					      	
				      }
				      ...
				      strcat(buff, " ");
				      strcat(buff, input_file_backup_path);
				      printf("buff = [%s]\n", buff);
				      system(buff);*/
