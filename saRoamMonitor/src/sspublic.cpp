...
int WriteLine(const int fd, FileBuff *fb, const char *src, const int length)
{
	...
	// ½«»º´æÖÐÎ´ÂúµÄÊý¾ÝÐ´µ½ÎÄ¼þ£¬ÒÔ±£Ö¤Ò»ÌõÍêÕûµÄ¼ÇÂ¼Èç¹ûÐèÒªÏòÎÄ¼þÐ´Èë£¬ÊÇÔÚ±¾º¯Êýµ÷ÓÃÒ»´ÎµÄ¹ý³ÌÖÐÍê³É
	/*if(fb->num != 0)
	{
...
int ReadConfig(char *filename, Config *cfg)
{
	...
	if(fd>0) {
		while( num>0 ) {
			...
			else if( !strcasecmp(word[0],"load_file_num"))
			{
			...
			else if( !strcasecmp(word[0],"load_interval"))
			{
			...
			else if( !strcasecmp(word[0],"load_file_prefix"))
			{
			...
			else if( !strcasecmp(word[0],"pros_file_path")){
				strncpy(cfg->pros_file_path, word[1], 200);
			}	
			...
			else if( !strcasecmp(word[0],"hlr_table_name")){
				strncpy(cfg->hlr_table_name, word[1], 50);
			}
			else if( !strcasecmp(word[0],"hlr_select_cond")){
				strncpy(cfg->hlr_select_cond, word[1], 100);
			}
			else if( !strcasecmp(word[0],"vlr_table_name")){
				strncpy(cfg->vlr_table_name, word[1], 50);
			}
			else if( !strcasecmp(word[0],"vlr_select_cond")){
				strncpy(cfg->vlr_select_cond, word[1], 100);
			}	
			else if( !strcasecmp(word[0],"roam_table_name")){
				strncpy(cfg->roam_table_name, word[1], 50);
			}	
			else if( !strcasecmp(word[0],"roam_select_cond")){
				strncpy(cfg->roam_select_cond, word[1], 100);
			}										
			...
			else if( !strcasecmp(word[0],"signal_check_interval")){
			   cfg->signal_check_interval = atoi(word[1]);	
			}
			else if( !strcasecmp(word[0],"signal_roam_judge_change")){
			   cfg->signal_roam_judge_change = atoi(word[1]);	
			}	
			else if( !strcasecmp(word[0],"signal_roam_judge_timing")){
			   cfg->signal_roam_judge_timing = atoi(word[1]);	
			}								
		}
		close(fd);
		...
		perror("Can't open saRoamMonitor.conf");
		return -1;
	}
}
...
{
	printf("\n");
	printf("ÏÔÊ¾ÅäÖÃÎÄ¼þsaRoamMonitor.confÖÐÐÅÏ¢¿ªÊ¼...\n");
	printf("analyze_thread_num:%d\n",cfg->analyze_thread_num);
	printf("load_file_num:%d\n",cfg->load_file_num);
	printf("hash_tune:%d\n",cfg->hash_tune);
	...
	printf("db_passwd:%s\n",cfg->db_passwd);
	printf("table:%s\n",cfg->table_name);
	printf("load_file_prefix:%s\n",cfg->load_file_prefix);
	printf("signal_sort_flag:%d\n",cfg->signal_sort_flag);
	printf("signal_sort_capacity:%d\n",cfg->signal_sort_capacity*10000);
	printf("signal_sort_interval:%d\n",cfg->signal_sort_interval);	
	printf("log_file_path:%s\n",cfg->log_file_path);				
	printf("hlr_table_name:%s \n",cfg->hlr_table_name);
	printf("hlr_select_cond:%s \n",cfg->hlr_select_cond);	
	printf("vlr_table_name:%s \n",cfg->vlr_table_name);
	printf("vlr_select_cond:%s \n",cfg->vlr_select_cond);	
	printf("roam_table_name:%s\n",cfg->roam_table_name);
	printf("roam_select_cond:%s\n",cfg->roam_select_cond);
	printf("signal_check_interval:%d\n",cfg->signal_check_interval);
				
	printf("ÏÔÊ¾ÅäÖÃÎÄ¼þsaRoamMonitor.confÖÐÐÅÏ¢½áÊø\n");
	printf("\n");
}
...
// È¡µÃÏµÍ³µ±Ç°Ê±¼ä
// str_sys_time ·µ»Ø"1970-01-01 08:00:00"¸ñÊ½×Ö·û´®
// º¯Êý·µ»ØÖµÎªÃëÊý
...
// È¡µÃÏµÍ³µ±Ç°Ê±¼ä
// str_sys_time ·µ»Ø"19710203040506"¸ñÊ½×Ö·û´®
// º¯Êý·µ»ØÖµÎªÃëÊý
...
int write_error_file(char *filepath,char *modulename,char *errcode,int level,char *errinfo)
{
  ...
  //sprintf(line,"%s,%s,%s,%d,%s,%s,%s,%d,%s",chk.host_code,chk.entity_type_code,chk.entity_sub_code,getpid(),modulename,str_begin_time,errcode,level,errinfo);      
...
//int write_log_file(char *filepath,char *modulename,char *file_path,char *file_name,char *begin_time,char *end_time,int duration,int counts,int size){
int write_log_file(char *filepath,char *modulename,char *msgcode,char *file_name,char *end_time,char *msgvalue){
  ...
  //sprintf(line,"%s%s%s%d%s%s%s%s%s",chk.host_code,chk.entity_type_code,chk.entity_sub_code,getpid(),modulename,end_time,msgcode,file_name,msgvalue);      
  sprintf(line,"%s	%s	%s	%d	%s	%s	%s	%s								 		%s	\n",chk.host_code,chk.entity_type_code,chk.entity_sub_code,getpid(),modulename,end_time,msgcode,file_name,msgvalue);    
...
int write_other_file(char *filepath,char *modulename,char *errinfo)
{
  int ret = 1 ;
	FILE	*fp;
  char filename[250];
  ...
  memset(filename,'\0',sizeof(filename));
  sprintf(filename,"%s%s_%s_%s.log",filepath,modulename,"transit",str_begin_time);
  if((fp=fopen(filename,"a+"))==NULL){
      printf("Cannot write file!");
      ret = 0 ;
  }
  
  if(fputs(errinfo,fp) == NULL){
      printf("Cannot fputs data to file!");
      ret = 0 ;  	  
  }
  fclose(fp);
...
// ¸ù¾ÝÊäÈëÊ±¼ä£¬¶Ô·ÖÖÓºÍÃëÖÓ²¿·ÖÇåÁã
// »ñÈ¡Ð¡Ê±¼¶µÄÊ±¼ä´Á
// str_time ·µ»Ø"20100315100000"¸ñÊ½×Ö·û´®
// º¯Êý·µ»ØÖµÎªtime_t
...
// ¸ù¾ÝÊäÈëÊ±¼ä£¬×ª»»ÎªYYYYMMDDHHMMSS¸ñÊ½
// str_time ·µ»Ø"20100315100000"¸ñÊ½×Ö·û´®
// º¯Êý·µ»ØÖµÎªtm
